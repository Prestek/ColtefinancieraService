...
class QuoteServiceTest {
    ...
    @Test
    @DisplayName("Should return bank code COLTEFINANCIERA")
    void shouldReturnBankCodeColtefinanciera() {
        // When
        QuoteDto quote = quoteService.quote(testRequest);

        // Then
        assertThat(quote.institution()).isEqualTo("COLTEFINANCIERA");
    }
    ...
    void shouldGenerateQuoteWithAppropriateRateForHighDTI() {
        ...
        assertThat(quote).isNotNull();
        assertThat(quote.aprEAEstimated()).isGreaterThan(0);
        // Coltefinanciera tiene wDTI = 0.30
    }
    ...
    @Test
    @DisplayName("Should verify Coltefinanciera specific weights")
    void shouldVerifyColtefinancieraSpecificWeights() {
        // Coltefinanciera tiene weights específicos:
        // wScore = 0.45 (muy sensible al puntaje)
        // wDTI = 0.30
        
        // Given - Bajo puntaje debería afectar significativamente
        QuoteRequest lowScore = new QuoteRequest(
                10000000L, 36, 600, 5000000L, 1000000L  // Score bajo
        );
        
        QuoteRequest highScore = new QuoteRequest(
                10000000L, 36, 800, 5000000L, 1000000L  // Score alto
        );

        // When
        QuoteDto quoteLowScore = quoteService.quote(lowScore);
        QuoteDto quoteHighScore = quoteService.quote(highScore);

        // Then
        double rateDifference = quoteLowScore.aprEAEstimated() - quoteHighScore.aprEAEstimated();
        assertThat(rateDifference).isGreaterThan(0); // Menor score = mayor tasa
    }
}
